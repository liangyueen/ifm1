...
public class RedeemSaveSubmitAction  implements ICommonAction {
	...
	public Object doAction(IRequest request) {
		...
		try {
			...
			Logger.debug("开始调用动作脚本 ActionName[" + getSaveActionCode() + "] BillType[" + getBillTypeCode() + "]...");
			vos = (AggInvestRedeemVO[])getCPFlowService().exeScriptPFlow(context);
			Logger.debug("调用动作脚本 ActionName[" + getSaveActionCode() + "] BillType[" + getBillTypeCode() + "]结束");
			vos = getEQueryService().getAggVOsByPKs(vos[0].getParentVO().getPk_redeem());
			
			// 提交处理
			// 判断单据是否配置有审批流选择执行对应的动作脚本动作脚本
			boolean hasFlow = NCCFlowUtils.hasApproveflowDef(getBillTypeCode(), vos[0]);
			// 构建动作脚本执行所需参数
			...
			Logger.debug("开始调用动作脚本 ActionName[" + getSubmitActionCode() + "] BillType[" + getBillTypeCode() + "]...");
			Object[] results = null;
			if (hasFlow) {
				// 有审批流的单据提交
				...
				results = getCPFlowService().exeScriptPFlow_CommitNoFlowBatch(commitcontext);
				PfProcessBatchRetObject retObject = (PfProcessBatchRetObject) results[0];
				// 执行动作脚本出错捕获出错信息
				if (retObject.getRetObj() == null || retObject.getRetObj().length == 0) {
			...
			Logger.debug("调用动作脚本 ActionName[" + getSubmitActionCode() + "] BillType[" + getBillTypeCode() + "]结束");
	 ...
	 * @return
	 */
	protected AggInvestRedeemVO[] doBefore(AggInvestRedeemVO[] vos)  throws BusinessException {
		...
		//	vo.setAttributeValue("vbillstatus", vbillstatus);
			...
			// 校验申请编号
			AggInvestRedeemVO[] oldvo = getEQueryService().getAggVOsByPKs(new String[]{ vo.getPk_redeem() });
	...
	protected BillCard doReturn(AggInvestRedeemVO[] vos) {
		// 把结果进行封装返回
		BillCardConvertProcessor processor = new BillCardConvertProcessor();
		BillCard billCard = processor.convert(TMIFMConst.CONST_PAGECODE_CONTRACT_CARD, vos[0]);
		// 卡片参照翻译
		...
		try {
			this.doBefore(operaVO);
			 ...
			 try {
					...
					AggInvestRedeemVO[] vos = (AggInvestRedeemVO[]) result;
					list.add(vos[0]);
				} catch (BusinessException e) {
