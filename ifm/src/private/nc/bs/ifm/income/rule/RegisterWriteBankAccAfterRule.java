...
import nc.vo.ifm.income.IFACIFMAccSuper;
...
public class RegisterWriteBankAccAfterRule extends IFACIFMAccSuper implements
	...
	public void writeBankAcc(AggregatedValueObject aggVO, String org,
		...
		if (null != headvo.getGathering()) {
			...
			// 给BankDeatailVO赋值
			initBankDeatailVO(detailVO, headvo,headvo.getGathering(), CmpConst.Direction_Pay);
			// 调用共同赋值逻辑
	...
	private void initBankDeatailVO(BankAccDetailVO detailVO,
			InvestIncomeVO headvo, String account, int sf) {
		// 明细表主键
		detailVO.setPk_bill_b(headvo.getPk_income());
		// 本方账户(通知账户/结算账户)
		detailVO.setPk_account(headvo.getGathering());
		// 来源单据类型
		...
		// 付款原币金额
		detailVO.setPaymoney(headvo.getOlcmoeny());
		// 记账人
		detailVO.setTallyperson(headvo.getBillmaker());
		// 记账时间
		detailVO.setTallytime(new UFDate(headvo.getBillmakedate().toLocalString()));
		// 记账日期
		detailVO.setTallydate(new UFLiteralDate(headvo.getBillmakedate().toLocalString()));
		// 付款本币金额
		detailVO.setOlcpaymoney(headvo.getActualmoeny());
		// 唯一线索号:
		// 单据编号 + 主表主键 + 子表主键（没有子表写主表主键）+ 账户PK +
		// 收付方向（现金提供的常量）+特殊标识（出现一个账户写多笔同支或者同收方向的，自己定义特殊标识常量）+flag
		detailVO.setOnlyclueno(headvo.getVbillno() + headvo.getPk_income()
				+ account + sf);
	}
